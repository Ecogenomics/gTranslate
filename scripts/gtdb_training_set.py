...
class GtdbTrainingSet(object):
    ...
    def __init__(self):
        ...
        # list of species with reassigned stop codon that should be included in training
        # set using more relaxed QC criteria
        self.GTDB_REASSIGNED_SP = set(['s__Zinderia insecticola'])
        self.NCBI_REASSIGNED_SP = set(['s__Candidatus Hodgkinia cicadicola', 's__Candidatus Nasuia deltocephalincola', 's__Candidatus Stammera capleta'])
        self.NCBI_REASSIGNED_SP.update(set(['s__Candidatus Organicella extenuata', 's__Candidatus Pinguicoccus supinus']))
        self.NCBI_REASSIGNED_SP.update(set(['s__Hodgkinia cicadicola', 's__Nasuia deltocephalincola', 's__Stammera capleta']))
        self.NCBI_REASSIGNED_SP.update(set(['s__Organicella extenuata', 's__Pinguicoccus supinus']))

        # species appears to have incorrect spelling at NCBI
        self.NCBI_REASSIGNED_SP.update(set(['s__Candidatus Nasuia deltocephalinicola', 's__Nasuia deltocephalinicola']))

        # Must include the Fastidiosibacteraceae XS4 species cluster once (if) this genome appears in NCBI Assembly DB:
    ...
    def parse_qc_failed_file(self, gtdb_qc_failed_file: str, ncbi_taxonomy: dict) -> tuple[dict, dict]:
        ...
        with open_file(gtdb_qc_failed_file, 'rt') as f:
            ...
            for line in f:
                ...
                gtdb_sp = [t.strip() for t in gtdb_taxonomy.split(';')][-1]
                ncbi_sp = tokens[ncbi_sp_idx]

                if ncbi_sp in self.NCBI_REASSIGNED_SP:
                    assert ncbi_sp.replace('s__', '') in ncbi_taxonomy[gid]
    ...
    def run(self, gtdb_bac_metadata_file: str, 
        ...
        gtdb_reassigned_gids, sp_counts = self.parse_qc_failed_file(gtdb_qc_failed, ncbi_taxonomy)
